###################################################################################################
# draw figures of sensitivity analyses using the moving window of 3*3 grid cells (focal + 8 nearest neighborin cells)
...
load("models/beta_beta.deviation_envs_nn8.RDATA")
load("models/MM_SAR_beta_beta.deviation_envs_nn8.RDATA")
load("data/tree_pam/tree_pam6_final.RDATA")


####################
# figure: maps showing patterns of beta-diversity
...
tmap_save(beta.maps, file="results/Fig.S_Observed_beta_nn8_maps.png", unit="mm", width=180, height=100)



####################
# figure: maps showing patterns of residuals of observed phylogenetic and functional turnover and nestedness from the random expected
# the values to map 
...
titles <- c("A Deviation of phylogenetic turnover", "B Deviation of functional turnover",
            "A Deviation of phylogenetic nestedness", "B Deviation of functional nestedness")
...
tmap_save(beta.dev.maps, file="results/Fig.S_Deviation_PBD.FBD_nn8_maps.png", unit="mm",width=150, height=67)



####################
# figure: plots showing standardized coefficients from models regressing beta-diversity variables against environmental variables 
...
plot_coef_obsbeta <- ggplot(mmsar_coef_beta.obs) + 
  ...
  geom_point(aes(y = variable, x = coef.avg.cod, shape = fct_rev(beta_comp), fill = beta_signif, color = beta_signif),
             position = position_dodge(0.6), size = 3) + 
  geom_vline(xintercept = 0, linetype = 2, size = 0.3) + 
  scale_x_continuous(limit = c(-0.85, 0.85)) +
  labs(y = NULL, x = "Standardized coefficient") +  
  scale_shape_manual(name = NULL, values = beta_comp_shape) + 
...
ggsave(file="results/Fig.S_coefficients_observed_beta_nn8.png", unit="mm",width=150, height=150)


####################
# figure: plots showing standardized coefficients from models regressing deviation of phylogenetic and functional
...
plot_coef_betadev <- ggplot(mmsar_coef_beta.dev) + 
  ...
  geom_point(aes(y = variable, x = coef.avg.cod, shape = fct_rev(beta_comp), fill = beta_signif, color = beta_signif),
             position = position_dodge(0.6), size = 3) + 
  geom_vline(xintercept = 0, linetype = 2, size = 0.3) + 
  scale_x_continuous(limit = c(-0.8, 0.8)) +
  labs(y = NULL, x = "Standardized coefficient") +  
  scale_shape_manual(name = NULL, values = beta_comp_shape) + 
  scale_color_manual(name = NULL, values = beta_comp_color, guide ="none") + 
  scale_fill_manual(name = NULL, values = beta_comp_color, guide ="none") + 
...
cowplot::plot_grid(plot_coef_betadev, plot_grid(NULL, legend_betadev_comp, rel_widths =  c(0.2, 1)),
                   nrow = 2,  rel_heights =  c(1, 0.1))
ggsave(file="results/Fig.S_coefficients_beta_deviation_nn8.png", unit="mm",width=120, height=120)
